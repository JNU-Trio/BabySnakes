...
bool DisappearSnakePlay::init() {
	...
	m_barrier = barrierfactory.createBarrier(this,BarrierFactory::BARRIER_TYPE_DISAPPEAR);

    // generate food
    FoodFactory *foodfactory = new DisappearFoodFactory();
	...
	snake = snakefactory.createSimpleSnake(this,SnakeFactory::SNAKEIMG_TYPE_FAHAI,2);
    m_snakes.push_back(snake);
    ...
    this->schedule(schedule_selector(DisappearSnakePlay::scheUpdate),m_updateTime);

    return true;
}

void DisappearSnakePlay::judge() {
	bool flag = true;
	...
	if (flag) {
		...
		if (m_snakes[0]->m_victorFlag)
			gameover = GameOverLayer::getScene(VirtualMap::SCENE_TYPE_DISAPPEAR,true);
		else
			gameover = GameOverLayer::getScene(VirtualMap::SCENE_TYPE_DISAPPEAR, false);
		CCDirector::sharedDirector()->pushScene(gameover);
	}
}

void DisappearSnakePlay::scheUpdate(float time) {
    for(int i=0; i<m_snakes.size(); i++) {
